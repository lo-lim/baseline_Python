...
print(sorted(names.items()))   # 아기이름을 기준으로 오름차순 정렬   #name.items()는 key와 value를 둘다 구하는 함수
# [('Aimy', 9778), ('Bob', 5887), ('Kelly', 7855), ('Mary', 10999),

# 1. 아기이름(key)을 기준으로 오름차순 정렬
print(sorted(names.items(), key= lambda x : x[0]))   #names.items() 즉 key, value 값이 lambda의 x로 전달
# [('Aimy', 9778), ('Bob', 5887), ('Kelly', 7855), ('Mary', 10999),
...
#왜 저 names.items()가 그냥 lambda의 x로 전달이 되는지 복습하면서 고민하고 강사님께 질문.